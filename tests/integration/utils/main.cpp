...
int main () {
    {
        ...
        TEST.Expect_Eq(static_cast<uint8_t>(0b1101), RESULT);
        RESULT = utils::ExtractBits<uint32_t, 3, 5>(VALUE); //0b11
        TEST.Expect_Eq(static_cast<uint8_t>(0b11), RESULT);
        RESULT = utils::ExtractBits<uint32_t, 13, 20>(VALUE); //0b11011110
        TEST.Expect_Eq(static_cast<uint8_t>(0b11011110), RESULT);
        RESULT = utils::ExtractBits<uint32_t, 28, 31>(VALUE); //0b1010
        TEST.Expect_Eq(static_cast<uint8_t>(0b1010), RESULT);
        RESULT = utils::ExtractBits<uint32_t, 0>(VALUE); //0b1
        TEST.Expect_Eq(static_cast<uint8_t>(0b1), RESULT);
    }
}
